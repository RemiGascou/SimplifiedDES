...
def keygen(key):
    ...
    subkey2 = p8(left_circular_shift(left_circular_shift(lkey, 1), 1) + left_circular_shift(left_circular_shift(rkey, 1), 1))
    #subkey1 = p8(left_circular_shift(p10(key), 1))
    #subkey2 = p8(left_circular_shift(p10(key), 2))
...
def fk(data, subkey):
    ...
    def F(R, subkey):
        ...
        return None
    def ors(a, b):
        print(" | [ORS ] Entering fk with a:", a, "and b :",b)
        if len(a) == len(b):
            output = ''.join([str(int(a[k]) ^ int(b[k])) for k in range(len(a))]) # ^ = xor
            print(" | [ORS ] Entering fk with output:", output)
            return output
        return None
    if len(data) == 8:
